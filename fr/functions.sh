...
say "Jarvis est relancée, on reprend... "
say "et si c'est pas $testpersonnalite2_chemin_nom dire: On recommence pour repartir à zéro..."
fi
...
echo "0" > $testpersonnalite2_chemin_etape;
return;
fi
...
echo "4" > $testpersonnalite2_chemin_etape;
return;
fi

if [[ "$testpersonnalite2_compteur" == "4" ]]; then 
testpersonnalite2_reponse="4: désir de régler vos problèmes: $order,"
testpersonnalite2_sauvreponse;

say "Question 5: Votre maison de rêve est-elle entourée d'une clôture ?";
echo "5" > $testpersonnalite2_chemin_etape;
return;
fi

if [[ "$testpersonnalite2_compteur" == "5" ]]; then
	if [[ "$order" =~ "non" ]]; then 
	testpersonnalite2_reponse="5: il n'y a pas de clôture, vous êtes extroverti. Les visiteurs sont bienvenus en tout temps."
	else
	testpersonnalite2_reponse="5: La présence d'une clôture indique une personnalité introvertie. Vous préférez inviter les gens que de les voir arriver sans invitation.";
	fi
testpersonnalite2_sauvreponse;
...
echo "6" > $testpersonnalite2_chemin_etape;
return;
fi

if [[ "$testpersonnalite2_compteur" == "6" ]]; then 
	if [[ "$order" =~ "fleur" || "$order" =~ "nourriture" || "$order" =~ "person" || "$order" =~ "gens" || "$order" =~ "monde" ]]; then
	testpersonnalite2_reponse="6: Vous êtes généralement heureux."
	else
	testpersonnalite2_reponse="6: Vous êtes généralement triste."
	fi
testpersonnalite2_sauvreponse;
...
echo "7" > $testpersonnalite2_chemin_etape;
return;
fi
...
if [[ "$testpersonnalite2_compteur" == "7" ]]; then 
	...
	testpersonnalite2_reponse="7: la durabilité de votre relation avec $testpersonnalite2_reponseQ1 est périssable "; fi
	if [[ "$order" =~ "porcelaine" || "$order" =~ "métal"  || "$order" =~ "verre" ]]; then 
	testpersonnalite2_reponse="7: la durabilité de votre relation avec $testpersonnalite2_reponseQ1 est durable "
	fi
testpersonnalite2_sauvreponse;
say "Question 8: Que faites-vous de cette tasse ?";
echo "8" > $testpersonnalite2_chemin_etape;
return;
fi

if [[ "$testpersonnalite2_compteur" == "8" ]]; then 
testpersonnalite2_reponse="8: Ce que vous faites avec la tasse représente votre attitude envers $testpersonnalite2_reponseQ1: $order,";
testpersonnalite2_sauvreponse;

say "Question 9: Vous marchez jusqu'aux abords de la propriété et vous vous trouvez face à un plan d'eau. Quelle est l'étendue du plan d'eau ?"
say "est ce une rivière, ruisseau, lac exétéra ?";
echo "9" > $testpersonnalite2_chemin_etape;
return;
fi


if [[ "$testpersonnalite2_compteur" == "9" ]]; then 
testpersonnalite2_reponse="9: L'étendue du plan d'eau est proportionnelle à votre libido: $order,";
testpersonnalite2_sauvreponse;

say "Question 10: Comment traverserez vous ce plan d'eau ?";
echo "10" > $testpersonnalite2_chemin_etape;
return;
fi

if [[ "$testpersonnalite2_compteur" == "10" ]]; then
testpersonnalite2_reponse="10: Le degré avec le quel vous vous mouillez pour traverser le plan d'eau est représentatif de l'importance de la sexualité dans votre vie: $order,";
testpersonnalite2_sauvreponse;

say "Voilà, je vais répondre à ce test question par question,"
say "dites ok entre chaque réponse pour obetenir la suite des résultats ok ?"
...
echo "11" > $testpersonnalite2_chemin_etape;
return;
fi
...
testpersonnalite2_sauvreponse() {
if [[ "$testpersonnalite2_chemin_nom" == "" ]]; then
testpersonnalite2_chemin_nom=`cat $testpersonnalite2_chemin_tot | sed  -e "s/_/ /g"`
else

	if [[ "$testpersonnalite2_repeteok" == "" ]]; then
	echo "$testpersonnalite2_reponse" >> $testpersonnalite2_chemin_noms_encours;
	else
	testpersonnalite2_repeteok=""
	fi
return;
fi
